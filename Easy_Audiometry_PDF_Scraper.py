...
# Datum van deze versie: 25 november 2023 
...
init()
a ='\nWelkom. De EasyPDF AudioScraper kan een Easy Data pdf-file met toon- en spraakaudiogram analyseren en een beschrijving maken.'
b ='\nDe beschrijving wordt opgeslagen in een textfile in uw downloadlocatie.'
c = '\nHet programma is niet altijd nauwkeurig. Controleer daarom altijd goed de beschrijving.'
d = '\n© 2023 Guido Cattani'
print(Fore.WHITE + Style.BRIGHT + a + b + c + d + '\n' + Style.RESET_ALL)
...
def type_hearing_loss(hearing_loss_data):
    ...
    extent_hl = ' dB (gemiddeld bij 1, 2 en 4 kHz)'
    ...
    elif LG_1_4 < 20 and LG_05_1 < 20: type_hearing_loss = '(nagenoeg) normale gehoordrempels, ' + str(LG_1_4) + extent_hl
    elif LG_1_4 < 20 and 20 <= LG_05_1 < 25: type_hearing_loss = 'nagenoeg normale gehoordrempels, ' + str(LG_1_4) + extent_hl
    elif LG_1_4 >= 20 and ABG == None: type_hearing_loss = '(vermoedelijk) perceptief / conductief / gemengd gehoorverlies, ' + str(LG_1_4) + extent_hl
    elif LG_1_4 >= 20 and ABG < 15: type_hearing_loss = 'perceptief gehoorverlies, ' + str(LG_1_4) + extent_hl
    elif LG_1_4 >= 20 and ABG > 15 and BG_05_1 <= 15: type_hearing_loss = 'conductief gehoorverlies, ' + str(LG_1_4) + extent_hl
    elif LG_1_4 >= 20 and ABG > 15 and 15 < BG_05_1 <= 20: type_hearing_loss = '(overwegend) conductief gehoorverlies, ' + str(LG_1_4) + extent_hl
    elif LG_1_4 >= 20 and ABG > 15 and BG_05_1 > 20: type_hearing_loss = 'gemengd gehoorverlies, ' + str(LG_1_4) + extent_hl
...
def short_hearing_loss(hearing_loss_data):
    ...
    if LG_1_4 == None: extent_hl = 'een zeer ernstig'
    elif LG_1_4 < LIGHT_EXTENT: extent_hl = ''
    elif LIGHT_EXTENT <= LG_1_4 < LIGHT_TO_MODERATE_EXTENT: extent_hl = 'een licht'
    elif LIGHT_TO_MODERATE_EXTENT <= LG_1_4 < MODERATE_EXTENT: extent_hl = 'een licht tot matig'
    elif MODERATE_EXTENT <= LG_1_4 < 55: extent_hl = 'een matig'
    elif MODERATE_TO_SEVERE_EXTENT <= LG_1_4 < SEVERE_EXTENT: extent_hl = 'een matig tot ernstig'
    elif SEVERE_EXTENT <= LG_1_4 < SEVERE_TO_VERY_SEVERE_EXTENT: extent_hl = 'een ernstig'
    elif SEVERE_TO_VERY_SEVERE_EXTENT <= LG_1_4 < VERY_SEVERE_EXTENT: extent_hl = 'een ernstig tot zeer ernstig'
    elif LG_1_4 > VERY_SEVERE_EXTENT: extent_hl = 'een zeer ernstig'
...
def save_results_to_downloads(results, pdf_infile):
    """
    Saves the formatted results to a text file in the User Downloads folder.
    ...
    try:
        ...
        downloads_dir = join(home_dir, "Downloads")
        
        # Define the file name
        f_name = patient_data(pdf_infile) +'_' + 'TestDat_' + test_date(pdf_infile) + '.txt'
 
        # Create the full path to the file in the "Downloads" folder
        file_path = join(downloads_dir, f_name)
...
def main(): 
    ...
    while True:
        ...
        exit_input = input(Fore.WHITE + Style.BRIGHT +"Druk op Enter voor de analyse van een andere file, of typ 'exit' en druk op Enter om te verlaten:\n"\
                           + Style.RESET_ALL)
